namespace HREngine.Bots
{
    ...
    public class Playfield
    {
        ...
        //add xz
        public void triggerAHeroGotDamage(bool ownHero)
        {
            foreach (Minion m in ((ownHero) ? this.ownMinions : this.enemyMinions))
            {
                if (m.name == CardDB.cardName.火焰狂人 && !m.silenced)
                {
                   // int dmg = (ownplay) ? this.getHeroPowerDamage(1) : this.getEnemyHeroPowerDamage(1);
                       //   this.minionGetDamageOrHeal(target, dmg);
               //    if (target.Hp <= 0) 
                   drawACard(CardDB.cardName.unknown,m.own);
                }
            }
       }
        ...
        //addxz
        public void enemyMinionsGetDamage(int damages, int exceptID = -1)
        {
