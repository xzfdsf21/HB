namespace HREngine.Bots
{
    ...
    public class PenalityManager
    {
        ...
        public int getAttackWithMininonPenality(Minion m, Playfield p, Minion target)
        {
            ...
            bool isLethalCheck = p.isLethalCheck;//add火车王
            retval += getAttackSecretPenality(m, p, target);
            //鲜血小鬼CS2_059 潜行在你的回合结束时，随机使另一个友方随从获得+1生命值。
        ...
        public int getAttackWithHeroPenality(Minion target, Playfield p)
        {
            ...
            //如果没有斩杀和 公正之剑EX1_366 在你召唤一个随从后，使其获得+1/+1，这把武器失去1点耐久度。
            ...
            {
                //埃提耶什KARA_13_26 特效：在你释放一个法术后，随机召唤一个法力值消耗相同的随从，失去1点耐久度
                ...
                case CardDB.cardName.vinecleaver:
                    if (!isLethalCheck)
                    {
                        if (target.isHero) return 0;
                         
                    }
                    break;
                //银剑
                case CardDB.cardName.银剑:
                    if (!isLethalCheck)
                    {
                        if (target.isHero) return 0;
                ...
                case CardDB.cardName.doomhammer:
                    ...
                    {
                        //石化武器CS2_045&&法力值足够       COMBO：已装备毁灭之锤，石化武器在手并且法力值足够，仅仅PEN+10
                ...
                case CardDB.cardName.gorehowl:
                    ...
                    break;
                //黄铜指虎CFM_631   在你的英雄攻击后，使你手牌中的一张随机随从牌获得+1/+1。
                case CardDB.cardName.brassknuckles:
                    ...
                    break;
                //浸毒匕首AT_132_ROGUEt   奇数贼武器
                case CardDB.cardName.poisoneddagger:
                    if (!isLethalCheck)
                    {
                        if (target.isHero) return 0;
            ...
            {
                int wAttack = weaponInHandAttackNextTurn(p);
                //英雄技能是浸毒匕首AT 132 ROGUE H1 || 匕首精通CS2 083b H1
            ...
            if (p.ownWeapon.Angr == 1 && p.ownHeroName == HeroEnum.thief)//如果盗贼拿攻1的武器
            {
        ...
        public int weaponInHandAttackNextTurn(Playfield p)
        {
            ...
            {
                //下回合能够挂刀的条件是有武器牌 和 法力值==这个回合最大法力值+1
        ...
        public int getPlayCardPenality(CardDB.Card card, Minion target, Playfield p)
        {
            ...
            //野性印记CS2_009 使一个随从获得嘲讽和+2/+2。
            //举例说明 retval=abuff + tbuff
        ...
        {
            CardDB.cardName name = card.name;
            //黑暗私语GVG_041 抉择：召唤5个小精灵；或者使一个随从获得+5/+5并具有嘲讽。   其中：GVG_041a为+5/+5嘲讽BUFF。
            if (name == CardDB.cardName.darkwispers && card.cardIDenum != CardDB.cardIDEnum.GVG_041a) return 0;//铺场优先
            int pen = 0;
            //buff enemy?

            //野蛮咆哮CS2_011  ||  嗜血CS2_046
            ...
            if (!target.isHero && !target.own)
            {
                ...
                foreach (Handmanager.Handcard hc in p.owncards)
                {
                    switch (hc.card.name)
                    {
                        ...
                        case CardDB.cardName.biggamehunter:
                            if (target.Angr + this.attackBuffDatabase[name] > 6) return 5; //花式解怪，亏卡
                            break;

                        //暗言术：灭EX1_622
                        case CardDB.cardName.shadowworddeath:
                            if (target.Angr + this.attackBuffDatabase[name] > 4) return 5;// //花式解怪，亏卡
                            break;
                        default:
                            break;
                    }
                }
                //严酷的监工EX1_603  || 怒火中烧EX1_607
                ...
                {
                    pen = 500;//给敌方加严酷的监工|| 怒火中烧，PEN=500
                }
            }
            //如果是我方随从
            if (!target.isHero && target.own)
                ...
                if (!m.Ready)
                {
                    switch (card.name)
                    {
                        ...
                        case CardDB.cardName.大地之力:
                            if (target.stealth)
                            {
                                if ((TAG_RACE)target.handcard.card.race == TAG_RACE.ELEMENTAL) return 3;
                                else return 7;
                            }
                            if ((TAG_RACE)target.handcard.card.race == TAG_RACE.ELEMENTAL && p.owncards.Count < 2) return 3;
                            return 50;
                    ...
                    return 50;
                                                         ...
                                                         //临时攻击BUFF
                if (m.Hp == 1 && !m.divineshild && !this.buffing1TurnDatabase.ContainsKey(name))//不太推荐给HP=1的随从加临时BUFF
                {
                    return 10;
                }
                if (m.Angr == 0)
                {
                    if (!m.silenced && m.handcard.card.deathrattle) return -8;//推荐给亡语随从
                    return -5;
                }
            }
        ...
        private int getTauntBuffPenality(CardDB.Card card, Minion target, Playfield p)
        {
            ...
            if (!target.isHero && !target.own)
            {
                ...
                foreach (Handmanager.Handcard hc in p.owncards)
                {
                    ...
                    if (hc.card.name == CardDB.cardName.theblackknight) return 0;//黑骑士花式解怪
                }
        ...
        private int getSilencePenality(CardDB.cardName name, Minion target, Playfield p)
        {
            ...
            if (target == null)
            {
                //铁喙猫头鹰CS2_203 || 破法者EX1_048 || 丛林守护者EX1_166
            ...
            if (target.own)
            {
                ...
                {
                    //如果是自我沉默数据库中的我方随从，则PEN -5
                    if (!target.silenced && this.OwnNeedSilenceDatabase.ContainsKey(target.name)) return -5;
                    //如果我方随从的状态异常 或有 敌方智慧祝福EX1_363 或有
                    //真言术：耀AT_013 选择一个随从。每当其进行攻击，为你的英雄恢复4点生命值。
            ...
            else//如果是敌方随从
            {
                ...
                if (this.silenceDatabase.ContainsKey(name))
                {
                    ...
                    //敌方那些具有：需要自我沉默的随从，除非随从有嘲讽效果，不然PEN+500
        ...
        private int getDamagePenality(CardDB.Card card, Minion target, Playfield p)
        {
            ...
            //aoe damage *************************************************************************************
            int aoeDamageType = 0;
            if (this.DamageAllEnemysDatabase.ContainsKey(name)) aoeDamageType = 1;//对所有敌方随从直接AOE伤害属1类

            //场面上有随从奥金尼灵魂祭司EX1_591 && 奶全场的数据库魔法
            else if (p.anzOwnAuchenaiSoulpriest >= 1 && HealAllDatabase.ContainsKey(name)) aoeDamageType = 2;//属于2类AOE
            else if (this.DamageAllDatabase.ContainsKey(name)) aoeDamageType = 3;//打全场所有随从或角色属3类AOE
            ...
            if (aoeDamageType > 0)
            {
                ...
                for (int i = 0; i < anz; i++)
                {
                    ...
                    if (aoeDamage >= m.Hp && !m.divineshild)
                    {
                        switch (name)
                        {
                            ...
                            case CardDB.cardName.demonwrath: 
                                if ((TAG_RACE)m.handcard.card.race == TAG_RACE.DEMON) continue;
                                break;
                            //血色净化者GVG_101 战吼：对所有具有亡语的随从造成2点伤害。
                    ...
                    else
                    {
                        ...
                        //古拉巴什狂暴者EX1_399        每当该随从受到伤害，便获得+3攻击力。
        ...
        private int getHealPenality(CardDB.cardName name, Minion target, Playfield p)
        {
            ...
            if (HealTargetDatabase.ContainsKey(name))
            {
                ...
                //奶的目标是我方英雄
                if (target.isHero && target.own)//关键点
                {
                    foreach (Handmanager.Handcard hc in p.owncards)
                    {
                        //开心的食尸鬼
                        if (hc.card.name == CardDB.cardName.happyghoul)
                               return 0;  
        ...
        private int getCardDrawPenality(CardDB.Card card, Minion target, Playfield p)
        {
            ...
            //视界术CS2_053 || 召唤宠物 GVG_017 抽一张牌。如果该牌是野兽牌，则其法力值消耗减少（4）点。
            ...
            switch (card.name)
            {
                //严正警戒BRM_001  抽两张牌。在本回合中每有一个随从死亡，该牌的法力值消耗就减少（1）点。
                case CardDB.cardName.solemnvigil:
                    ...
                    break;
                //麦迪文的残影GVG_005 复制你的所有随从，并将其置入你的手牌。
                case CardDB.cardName.echoofmedivh:
        ...
        private int getCardDiscardPenality(CardDB.cardName name, Playfield p)
        {
            ...
            if (this.cardDiscardDatabase.ContainsKey(name))
            {
                ...
                foreach (Handmanager.Handcard hc in p.owncards)
                {
                    ...
                    switch (hc.card.name)
                    {
                        //镀银魔像KAR_205   如果你弃掉了这张随从牌，则会召唤它。
                        case CardDB.cardName.silverwaregolem: pen -= 12; continue;

                        //加拉克苏斯之拳AT_022  当你使用或弃掉这张牌时，随机对一个敌人造成4点伤害。
        ...
        private int getSpecialCardComboPenalitys(CardDB.Card card, Minion target, Playfield p)
        {
            ...
            if (lethal && card.type == CardDB.cardtype.MOB)
            {
                ...
                {
                    //雷德·黑手BRM_029  战吼：如果你的手牌中有龙牌，则消灭一个传说随从。
                    ...
                    else
                    {
                        if ((this.HealTargetDatabase.ContainsKey(name) || this.HealHeroDatabase.ContainsKey(name) || this.HealAllDatabase.ContainsKey(name)))
                        {
                            ...
                            foreach (Minion mm in p.ownMinions)
                            {
                                //圣光护卫者 ||   神圣勇士AT_011    每当一个角色获得治疗，便获得+2攻击力。
            ...
            //rule for coin on early game跳幸运币
            if (p.ownMaxMana < 4 && card.name == CardDB.cardName.thecoin) //修改
            {
                foreach (Handmanager.Handcard hc in p.owncards)
                {
            ...
            switch (card.name)
            {
                ...
                case CardDB.cardName.bookwyrm:
                    ...
                    break;
                //污手街守护者CFM_062  嘲讽，战吼：使相邻的随从获得圣盾。
        ...
        private int getAttackSecretPenality(Minion m, Playfield p, Minion target)
        {
            ...
            if (p.enemyHeroStartClass == TAG_CLASS.ROGUE)//假设闪避
            {
        ...
        {
            //菌菇附魔师LOOT_388
            HealAllDatabase.Add(CardDB.cardName.菌菇附魔师, 2);
            ...
            //菌菇附魔师
            HealTargetDatabase.Add(CardDB.cardName.菌菇附魔师, 2);
        ...
        private void setupDamageDatabase()
        {
            ...
            //对所有角色造成伤害
            //憎恶EX1_097 
            DamageAllDatabase.Add(CardDB.cardName.abomination, 2);//全场打2
            //渊狱惩击者CFM_751
            DamageAllDatabase.Add(CardDB.cardName.abyssalenforcer, 3);//全场打3
            //阿诺玛鲁斯OG_120  亡语：对所有随从造成8点伤害。
        ...
        private void setupsilenceDatabase()
        {
            ...
            //心能魔像GVG_077 在每个回合结束时，如果该随从是你唯一的随从，则消灭该随从。
            ...
            //爆破小队CFM_667  战吼：对一个敌方随从造成5点伤害。亡语：对你的英雄造成5点伤害。
        ...
        {
            this.heroAttackBuffDatabase.Add(CardDB.cardName.大地之力, 2);
            ...
            this.attackBuffDatabase.Add(CardDB.cardName.凶猛狂暴, 4);
        ...
        {
            healthBuffDatabase.Add(CardDB.cardName.大地之力, 2);
            ...
            healthBuffDatabase.Add(CardDB.cardName.凶猛狂暴, 4);
        ...
        private void setupCardDrawBattlecry()
        {
            ...
            cardDrawBattleCryDatabase.Add(CardDB.cardName.我找到了, 1);
            cardDrawBattleCryDatabase.Add(CardDB.cardName.元素反应, 1);
            cardDrawBattleCryDatabase.Add(CardDB.cardName.亡鬼幻象, 1);
            cardDrawBattleCryDatabase.Add(CardDB.cardName.狗头人图书管理员, 1);
            cardDrawBattleCryDatabase.Add(CardDB.cardName.主人的召唤, 3);//抽牌数
            ...
            //十字军统领AT_118   战吼：随机将一张圣骑士牌置入你的手牌。
        ...
        {
            cardDiscardDatabase.Add(CardDB.cardName.darkbargain, 6);//数字为费用
        ...
        private void setupBuffingMinions()
        {
            ...
            //临时攻击BUFF
            buffing1TurnDatabase.Add(CardDB.cardName.abusivesergeant, 0);//叫嚣的中士
            buffing1TurnDatabase.Add(CardDB.cardName.bloodlust, 3);//嗜血
            buffing1TurnDatabase.Add(CardDB.cardName.darkirondwarf, 0);//黑铁矮人
            buffing1TurnDatabase.Add(CardDB.cardName.rockbiterweapon, 0);//石化武器
            buffing1TurnDatabase.Add(CardDB.cardName.worshipper, 0);//膜拜者
        }
        ...
        private void setupEnemyTargetPriority()
        {
            ...
            priorityTargets.Add(CardDB.cardName.梦境花栽种师, 10); 
            priorityTargets.Add(CardDB.cardName.狗头人幻术师, 10); 
            priorityTargets.Add(CardDB.cardName.荆棘帮暴徒, 10);
            priorityTargets.Add(CardDB.cardName.达利乌斯克罗雷, 10);
            priorityTargets.Add(CardDB.cardName.格林达鸦羽, 10); 
            priorityTargets.Add(CardDB.cardName.夜鳞龙后, 10); 
            priorityTargets.Add(CardDB.cardName.玻璃骑士, 10); 
            priorityTargets.Add(CardDB.cardName.大法师阿鲁高, 10);
            priorityTargets.Add(CardDB.cardName.三眼乌鸦, 10); 
            priorityTargets.Add(CardDB.cardName.驯犬大师肖尔, 10);
            priorityTargets.Add(CardDB.cardName.观星者露娜, 10); 
           
            priorityTargets.Add(CardDB.cardName.犀牛之灵, 4);
            priorityTargets.Add(CardDB.cardName.蝙蝠之灵, 4);
            priorityTargets.Add(CardDB.cardName.青蛙之灵, 4);
            priorityTargets.Add(CardDB.cardName.鲨鱼之灵, 4);
            priorityTargets.Add(CardDB.cardName.亡者之灵, 4);
            priorityTargets.Add(CardDB.cardName.猛虎之灵, 4);
            priorityTargets.Add(CardDB.cardName.龙鹰之灵, 4);
            priorityTargets.Add(CardDB.cardName.山猫之灵, 4);
            priorityTargets.Add(CardDB.cardName.迅猛龙之灵, 4);
            priorityTargets.Add(CardDB.cardName.水晶工匠坎格尔, 10);
            priorityTargets.Add(CardDB.cardName.影舞者索尼娅, 10);
            priorityTargets.Add(CardDB.cardName.女巫的坩埚, 10);
            priorityTargets.Add(CardDB.cardName.奥术工匠, 10);
            priorityTargets.Add(CardDB.cardName.火焰狂人, 10);
            ...
            priorityTargets.Add(CardDB.cardName.northshirecleric, 2);
        ...
        private void setupSilenceTargets()
        {
            ...
            silenceTargets.Add(CardDB.cardName.狗头人幻术师, 0);
            silenceTargets.Add(CardDB.cardName.贡克迅猛龙之神, 0); 
            silenceTargets.Add(CardDB.cardName.机械雏龙, 0);
            silenceTargets.Add(CardDB.cardName.荒疫爬行者, 0);
            silenceTargets.Add(CardDB.cardName.格林达鸦羽, 0);
            silenceTargets.Add(CardDB.cardName.破棺者, 0);
            silenceTargets.Add(CardDB.cardName.夜鳞龙后, 0);
            silenceTargets.Add(CardDB.cardName.紫色岩虫, 0);
            silenceTargets.Add(CardDB.cardName.食肉魔块, 0);
            silenceTargets.Add(CardDB.cardName.首席门徒林恩, 0);
            silenceTargets.Add(CardDB.cardName.虚空领主, 0);
            silenceTargets.Add(CardDB.cardName.着魔男仆, 0);
            silenceTargets.Add(CardDB.cardName.莱妮莎炎伤, 0);
            silenceTargets.Add(CardDB.cardName.机械蛋, 0);
            silenceTargets.Add(CardDB.cardName.奇利亚斯, 0); 
